...
##            count=z[0]
        ...
        def addnew():
            new1=Tk()
            new1.configure(bg='black')
##            new.geometry('200x300')
##            Label(new1).grid(row=0,column=0)
            Label(new1,text=' Enter another number  ',bg='black',fg='green').grid(row=1,column=0,sticky=W)
            a1=Entry(new1)
            a1.grid(row=1,column=1)
            Button(new1,text=' Add ',bg='black',fg='green').grid(row=2,column=1,sticky=E)
            
            
        Button(new,text='   +   ',bg='black',fg='green',command=addnew).grid(row=7,column=1,sticky=E)
        Label(new,text=' Insert a number (req) ',bg='black',fg='green').grid(row=7,column=0)
        ...
        def save():
            ...
            i=e9.get()
            j=e10.get()
####            dat=cur.execute('')           checking date from today's date
##            for x in dat:
##                print x[0]
            if len(e)==0:
                new.destroy()
